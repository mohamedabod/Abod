...
**قراءة البيانات — تحليل مكتوب على سجلك**
- **`personalAnalysis()`**: قراءة للسجل ككل، مش تطبيق قواعد عليه — بتقول اللي
  بيبان بس لما تبص للقطع وهي جنب بعض، وكمان بتشكّك في **البيانات نفسها** لما
  تكون هي المشكلة (قراءة ميزان مش متسقة، جرعة مكرّرة في السجل، تسجيل حالة في
  التوقيت الغلط). كل قسم بيقول: لقيت إيه، معناه إيه، وتعمل إيه.
- **`expertInsights()`**: ٩ قواعد تغذية وتدريب مرتّبة بأولوية، بتسكت لما البيانات
  مش كفاية تبرّر الكلام.
- **`insightLog`**: بيفضل فاكر إيه اللي اتقال ومتى، فيقدر يرجع بعد أسبوع يقولك
  إن الملاحظة دي اتحلّت. ده الفرق بين مدرب ومحرّك قواعد.
...
- **الأسبوع ده مقابل اللي فات**: ساعات الصيام والجلسات والمتوسط والخطوات والنوم
  والوزن — كفروق مش أرقام مجردة، وبينتهي بـ**حاجة واحدة تغيّرها**.
- **خط اتجاه الوزن (متوسط متحرك ٧ أيام)**: وزن الميزان بيتأرجح فوق كيلو في اليوم
  على مية وجليكوجين، لدرجة إن قراءتين خام ممكن يقلبوا إشارة الاتجاه. كل حاجة
  بتحكم على الاتجاه بتقرا الخط المنعّم مش النقط الخام.
- **الاستشفاء من نبض الراحة**: خط أساس شخصي (وسيط) + كشف الارتفاع المستمر.
  ٥٨ ممتازة لواحد ومرتفعة لواحد تاني — اللي بيفرق هو بعدك عن أساسك إنت.
- **صيام بأثر رجعي وتعديل السجل**: أي صيام التطبيق نام عليه بيتسجّل يدوي،
  وأي صف في السجل بيتعدّل. سجل مش بيتصلّح مش سجل — ده إيصال.
- **يوم فطار مخطط**: الفطار المقصود بيوصّل السلسلة من غير ما يزوّدها.
- **النبض مقابل الصيام**: كل نقطة يوم — ساعات صيامك أفقياً ونبضك رأسياً.
- **ويدجت للشاشة الرئيسية** بالعدّاد والمرحلة والتقدّم.
- **نسخة احتياطية تلقائية** كل أسبوع، و**تقرير شهري** بصيغتين: **PDF فيه صور
  وجباتك** (عن طريق نظام الطباعة في أندرويد — من غير أي مكتبة PDF، فالحجم زي ما
  هو)، ونسخة نصية للّصق في شات.

**التمارين — شرح مرئي**
- ٨ حركات مركّبة (سكوات · ضغط · سحب · رفعة رومانية · ضغط كتف · اندفاع · بلانك ·
  جسر الحوض)، كل واحدة معاها **رسم متحرك** وأربع نقاط أداء وأهم غلطة فيها.
- الرسم **SVG مرسوم بمفاصل** ومتحرك بـCSS transforms — مش فيديو ولا GIF. فيديو
  لتمانية تمارين كان هيبقى عشرات الميجابايت ومحتاج فك ترميز كل مرة؛ ده مية بايت
  markup للتمرين، بيتمدّد لأي شاشة، وبياخد لون الثيم. **الحجم زاد ١٤ كيلوبايت
  للتطبيق كله.**
- مدخله من صفحة المدرب، لأن ده بالظبط المكان اللي التطبيق بيقول فيه من أسابيع إن
  مفيش تمارين مقاومة.

**الحركة (Motion)**
- سلّم واحد: `--t1` للمسة، `--t2` للظهور، `--t3` لقيمة بتتحرك في رسم. كلهم
  مضروبين في `--motion` عشان `prefers-reduced-motion` يصفّرهم من مكان واحد.
- بس `transform` و`opacity` — مفيش أي حاجة بتحرّك خصائص بتعمل reflow.
- **٦٠ إطار/ثانية** أثناء صيام شغّال وصفر إطارات متأخرة، و٦٠ برضه مع تمانية رسوم
  متحركة على الشاشة (مقاسة، مش تخمين).

**وضع رمضان**
- النافذة بتبقى المغرب → الفجر، متحسبة فلكياً لإحداثياتك كل يوم، وكل التذكيرات
  بتتحرك معاها. طرق حساب: الهيئة المصرية (افتراضي) · رابطة العالم الإسلامي ·
  أم القرى · ISNA.

**المظهر**
- ثيم غامق/فاتح/حسب النظام، ٥ ألوان أساسية، و٤ أحجام خط — كلها CSS custom
  properties على `<html>`، فالتبديل لحظي ومفيش قيمة لون متكررة في مكانين.

**الماء**
- الهدف بيتحسب (٣٥ مل/كجم + نص لتر لكل ساعة تمرين + بدل جو حر) بدل ٣ لتر ثابتة
  للكل. ٣ لتر غلط في الاتجاهين: كتير على جسم صغير قاعد، وقليل على ٩٦ كجم بيركب
  ٢٠ كيلو في صيف إسكندرية.
...
app/src/main/assets/public/
  ...
  RouteTracker.java            تتبّع المسار + آخر موقع معروف (لمواقيت الشمس)
  PdfExporter.java             HTML -> PDF عبر PrintManager (من غير مكتبات)
  PrintTask.java               ينقل التصدير لخيط الواجهة
  Reminders.java               التنبيهات اليومية + خانتين للنصايح المولّدة
  utils.js                     التخزين، i18n، المراحل، الأكل، الحسابات،
                               محرّك التحليل، حساب الشمس، الجسر
...
JS  -> Native   syncFast, bandScan, bandConnectSaved, bandDisconnect, bandAuto,
                ...
                setInsight, lastLocation, healthSync, photoCapture, routeStart
